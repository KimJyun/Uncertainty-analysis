...
def checkpoint(model, best_loss, epoch, LR, filename):
    ...
    torch.save(state, '8results/' + filename)
...
def train_model(
    ...
    tensorboard_writer_train = SummaryWriter('runs/jh/8/train_loss2') #����
    # tensorboard_writer_val = SummaryWriter('runs/loss/val_loss')
    tensorboard_writer_val = SummaryWriter('runs/jh/8/val_loss2')#����
    ...
    if not regression:
        ...
        tensorboard_writer_mae = SummaryWriter('runs/jh/8/mae')
    ...
    if not fine_tune:
        ...
        checkpoint_best = torch.load('8results/regression_checkpoint_best')
