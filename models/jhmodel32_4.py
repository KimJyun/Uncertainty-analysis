...
def checkpoint(model, best_loss, epoch, LR, filename):
    ...
    torch.save(state, '4results/' + filename) #모델 가중치 저장
...
def train_model(
    ...
    tensorboard_writer_train = SummaryWriter('runs/train_loss') 
    # tensorboard_writer_val = SummaryWriter('runs/loss/val_loss')
    tensorboard_writer_val = SummaryWriter('runs/val_loss')
    ...
    if not regression:
        ...
        tensorboard_writer_mae = SummaryWriter('runs/jh/4/mae')
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        with open("4results/log_train", 'a') as logfile:
    ...
    if not fine_tune:
        ...
        checkpoint_best = torch.load('4results/regression_checkpoint_best')
        ...
        if fine_tune and not initial_brixia_model_path:
            ...
            if not regression:
                ...
                model.module.classifier = nn.Sequential(
                    ...
                    # nn.Dropout2d(drop_rate) #dropout 추가
                )
    else:
