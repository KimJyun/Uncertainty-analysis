...
def checkpoint(model, best_loss, epoch, LR, filename):
    ...
    torch.save(state, '16results/' + filename)
...
def train_model(
    ...
    tensorboard_writer_train = SummaryWriter('runs/jh/16/train_loss2') #����
    # tensorboard_writer_val = SummaryWriter('runs/loss/val_loss')
    tensorboard_writer_val = SummaryWriter('runs/jh/16/val_loss2')#����
    ...
    if not regression:
        ...
        tensorboard_writer_mae = SummaryWriter('runs/jh/16/mae')
    ...
    if not fine_tune:
        ...
        checkpoint_best = torch.load('16results/regression_checkpoint_best')
