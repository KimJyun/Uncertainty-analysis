...
use_gpu = torch.cuda.is_available() #gpu ��밡������ ���� äũ
...
def checkpoint(model, best_loss, epoch, LR, filename):
    ...
    torch.save(state, '128results/' + filename)
...
def train_model(
    ...
    tensorboard_writer_train = SummaryWriter('runs/jh/128/train_loss2') #����
    # tensorboard_writer_val = SummaryWriter('runs/loss/val_loss')
    tensorboard_writer_val = SummaryWriter('runs/jh/128/val_loss2')#����
    ...
    if not regression:
        ...
        tensorboard_writer_mae = SummaryWriter('runs/jh/128/mae')
    ...
    if not fine_tune:
        checkpoint_best = torch.load('results/checkpoint_best2')
    elif fine_tune and not regression:
        checkpoint_best = torch.load('results/classification_checkpoint_best')
    else:
        checkpoint_best = torch.load('128results/regression_checkpoint_best')
