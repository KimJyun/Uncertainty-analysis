...
def checkpoint(model, best_loss, epoch, LR, filename):
    ...
    torch.save(state, '14results/' + filename) #모델 가중치 저장
...
def train_model(
    ...
    if not regression:
        ...
        tensorboard_writer_mae = SummaryWriter('runs/jh/14/mae')
    ...
    if not fine_tune:
        ...
        checkpoint_best = torch.load('14results/regression_checkpoint_best')
